...
public class Unit_archer2 extends GameObject implements EntityUnit{ // 궁병에 대한 클래스
	 
	private double attack_point = 5.0; // 공격력
	private double health_point = 100.0; // 체력
	private double speed_point = 100.0; // 속도
	private int identifier = 2; // 1이면 아군 유닛, 2이면 상대방 유닛
	...
	public Unit_archer2(double x, double y, double enemy_x, double enemy_y, Textures tex, Controller controller, Game game)
	{
		super(x, y);
	...
	public void tick() {
		...
		for(int i = 0; i < game.eb.size(); i++ ) {
			...
			if(Physics.Collision(this, tempEnt) && tempEnt.check_identifier() == 1) { // 해당 유닛과 적군의 유닛이 충돌했을경우
				move = false;
